...
public class CarDeformation : MonoBehaviour
{
    ...
    [Tooltip("La force minimale du choc pour froisser la t�le")]
    public float minForceToDamage = 3.0f;
    [Tooltip("Le rayon d'impact (taille du crat�re)")]
    ...
    [Header("Debug")]
    [Tooltip("Laiss� vide, le script trouvera la carrosserie tout seul.")]
    ...
    // --- RECHERCHE AUTOMATIQUE INT�LLIGENTE ---
    private void AutoFindCarBody()
    {
        // Si tu as quand m�me mis un truc � la main, on le garde
        ...
        foreach (MeshFilter mf in allFilters)
        {
            ...
            // La carrosserie est g�n�ralement l'objet avec le plus de d�tails (vertices)
        ...
        if (bestFilter != null)
        {
            ...
            Debug.Log($"<color=green>[CarDeformation] Carrosserie trouv�e auto : {bestFilter.gameObject.name}</color>");
        }
    }
    ...
    private void InitializeMesh()
    {
        ...
        {
            Debug.LogError($"[CarDeformation] �CHEC : Aucune carrosserie trouv�e sur {gameObject.name}.");
            return;
        }
        ...
        {
            Debug.LogError($"[CarDeformation] ERREUR : Le mod�le '{originalMesh.name}' n'est pas modifiable ! Cochez 'Read/Write Enabled' dans l'onglet Model du fichier 3D.");
            return;
        }
    ...
    // Remet la carrosserie dans son état d'origine (vertices intacts) —
    // appelée par TuningShopManager.RepairCar() pour que la réparation soit
    // aussi visible qu'elle est mécanique.
